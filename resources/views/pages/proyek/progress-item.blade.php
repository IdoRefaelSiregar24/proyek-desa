<div class="accordion faq-accordion" id="accordionTahap{{ $proyek->proyek_id }}-{{ $t->tahap_id }}">

    <div class="accordion-item">
        <!-- HEADER -->
        <h2 class="accordion-header" id="heading{{ $t->tahap_id }}">
            <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse"
                data-bs-target="#collapse{{ $t->tahap_id }}" aria-expanded="false"
                aria-controls="collapse{{ $t->tahap_id }}">
                Tahapan : {{ $t->nama_tahap }}
            </button>
        </h2>


        <!-- BODY -->
        <div id="collapse{{ $t->tahap_id }}" class="accordion-collapse collapse"
            aria-labelledby="heading{{ $t->tahap_id }}" data-bs-parent="#accordionTahap{{ $t->tahap_id }}">
            ...
            <div class="accordion-body">
                ...
                <p class="text-muted mb-4">Lihat semua media terkait tahapan <strong>{{ $t->nama_tahap }}</strong>
                    di bawah ini.
                </p>
                            ...
                            <div class="card shadow-sm rounded overflow-hidden">
                                <a href="{{ asset('storage/' . $thumbnail->file_name) }}" class="glightbox"
                                    ...
                                    <img src="{{ asset('storage/' . $thumbnail->file_name) }}"
                                        alt="Thumbnail {{ $t->nama_tahap }}" class="img-fluid"
                ...
                <div class="section-title d-flex flex-wrap justify-content-between align-items-center gap-3">
                    <form method="GET" action="" class="d-flex flex-wrap align-items-center gap-2">
                        ...
                        <!-- Filter Tanggal -->
                        <input type="date" name="progress_mulai" value="{{ request('progress_mulai') }}"
                            class="form-control">
                        <input type="date" name="progress_selesai" value="{{ request('progress_selesai') }}"
                            class="form-control">

                        <!-- Filter Persen Real -->
                        <input type="number" name="persen_min" value="{{ request('persen_min') }}"
                            class="form-control" placeholder="Persen Min" min="0" max="100">
                        <input type="number" name="persen_max" value="{{ request('persen_max') }}"
                            class="form-control" placeholder="Persen Max" min="0" max="100">
                ...
                @forelse ($progressPaginated as $p)
                    <div class="position-relative ps-4 mb-4">
                        ...
                        <div class="card shadow-sm border-0 mb-4 rounded-4 overflow-hidden">
                            {{-- ====================== HEADER ====================== --}}
                            <div
                                class="d-flex justify-content-between align-items-center px-3 py-3 bg-light border-bottom">
                                <div>
                            ...
                            <div class="card-body pb-3">
                                    ...
                                    <div class="row g-3 mb-3">
                                        ...
                                        @foreach ($otherMedia as $media)
                                            ...
                                            <div class="col-4 col-md-2">
                                                ...
                                                @else
                                                    <div
                                                        class="p-2 small bg-light rounded border shadow-sm text-center">
                                                        <i class="fa-regular fa-file fs-4"></i>
                                                        <p class="small text-truncate mt-1">
                                                            {{ $media->file_name }}</p>
                                                    </div>
                                                @endif
                                            </div>
                                ...
                                @if ($medias->count() == 0)
                                    <p class="text-center text-muted">Belum ada dokumentasi media untuk progres
                                        ini.</p>
                                @endif
                ...
                <div class="d-flex justify-content-center gap-1">
                    @for ($i = 1; $i <= $progressPaginated->lastPage(); $i++)
                        <button onclick="loadProgress({{ $t->tahap_id }}, {{ $i }})"
                            class="btn btn-sm pagination-button {{ $i == $progressPaginated->currentPage() ? 'btn-primary' : 'btn-light' }}">
                            {{ $i }}
                        </button>
                    @endfor
