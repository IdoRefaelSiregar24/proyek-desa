...
@section('content')
    ...
    <div class="contact-form wow fadeInUp" data-wow-delay="0.25s">
        ...
        <form action="{{ route('tahapan-guest.update', [$proyek->proyek_id, $tahapan->id]) }}" method="POST">
            @csrf
            ...
            <div class="row">
                ...
                <div class="form-group col-md-3 mb-4">
                    <input type="number" step="0.01" name="target_persen" class="form-control"
                        placeholder="Target (%)" value="{{ old('target_persen', $tahapan->target_persen) }}" required>
                    @error('target_persen')
                        <div class="text-danger">{{ $message }}</div>
                    @enderror
                </div>
